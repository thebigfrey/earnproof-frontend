    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <h2 id="webhook-secret-heading" className="text-lg font-semibold text-emerald-100">
            Webhook Created Successfully
          </h2>
          <p className="mt-1 text-sm text-emerald-200">
            Your webhook endpoint for <strong>{webhook.url}</strong> has been created.
            <strong className="block mt-1">This is the only time the signing secret will be displayed.</strong>
            Copy it now and store it securely. Use it to verify webhook signatures on your endpoint.
          </p>

          <div className="mt-4 grid gap-3">
            <div>
              <label htmlFor="webhook-secret" className="block text-xs font-semibold text-emerald-100">
                Signing Secret
              </label>
            ...
            <div className="grid gap-2 text-xs text-emerald-200">
              <div>
                <span className="font-semibold">Webhook ID:</span> {webhook.id}
              </div>
              <div>
                <span className="font-semibold">URL:</span> {webhook.url}
              </div>
              <div>
                <span className="font-semibold">Events:</span> {webhook.events.join(", ")}
              </div>
              <div>
                <span className="font-semibold">Status:</span> {webhook.status}
              </div>
            </div>
          </div>
